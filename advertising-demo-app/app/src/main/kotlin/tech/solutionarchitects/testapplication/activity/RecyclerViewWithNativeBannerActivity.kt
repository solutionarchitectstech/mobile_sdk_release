...
import tech.solutionarchitects.testapplication.activity.recycler_native.Item
import tech.solutionarchitects.testapplication.activity.recycler_native.NativeBannerAdapter
import tech.solutionarchitects.testapplication.databinding.ActivityRecyclerViewWithNativeBannerBinding
import tech.solutionarchitects.testapplication.utils.showDebugMessage

class RecyclerViewWithNativeBannerActivity : AppCompatActivity() {

    private lateinit var binding: ActivityRecyclerViewWithNativeBannerBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityRecyclerViewWithNativeBannerBinding.inflate(layoutInflater)
    ...
    override fun onResume() {
        ...
        binding.recyclerView.adapter =
            NativeBannerAdapter(lifecycle = lifecycle, dataSet = items) { event ->
        ...
        get() {
            return mutableListOf<Item>().also { list ->
                repeat(250) {
                    if (it % 5 == 0) {
                        list.add(
                            Item.NativeBannerItem(
                                nativeBannerView = NativeBannerView(this).apply {
